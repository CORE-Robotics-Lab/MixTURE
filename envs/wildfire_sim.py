...
class WildFire(object):
    ...
    def fire_intensity(
        ...
        x_dev = (
            self.rng.integers(low=deviation_min, high=deviation_max, size=(1, 1))[0][0]
            + self.rng.normal()
        )
        y_dev = (
            self.rng.integers(low=deviation_min, high=deviation_max, size=(1, 1))[0][0]
    ...
    def geo_phys_info_init(
        ...
        spread_rate = (
            fuel_rng
            * self.rng.random(size=(self.terrain_sizes[0], self.terrain_sizes[1]))
            + min_fuel_coeff
        )
        wind_speed = self.rng.normal(avg_wind_speed, 2, size=(self.terrain_sizes[0], 1))
        wind_direction = self.rng.normal(
    ...
    def fire_propagation(
        ...
        for point in ign_points_all:
            ...
            if (
                ...
                U = wind_speed[self.rng.integers(low=0, high=self.terrain_sizes[0])][0]
                Theta = wind_direction[
                    self.rng.integers(low=0, high=self.terrain_sizes[0])
                ][0]
