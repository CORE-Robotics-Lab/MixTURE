...
def trial(config):
    ...
    envs = {
        "predatorprey": (
            "Predator-Prey",
            PredatorPrey(20, 10, vision=2, max_timesteps=80),
        ),
        "predatorcapture": (
            "Predator-Capture-Prey",
            PredatorCapture(20, 6, 4, vision=2, max_timesteps=80),
        ),
        "firecommander": (
            "FireCommander",
            FireCommander(20, 6, 4, vision=2, max_timesteps=80),
        ),
    }
    ...
    if config["method"] == "RL":
        ...
        demo_filename = f"../demos/{config['env']}_20x20_comm.pickle"
        env = add_uniform_comms(env, comm_dim)
        reward = ExplicitReward()
    elif config["method"] == "no-comm LFD":
        agents = RecurrentAgents(lr, lr, fc_dim=256)
        demo_filename = f"../demos/{config['env']}_20x20.pickle"
    ...
    os.makedirs(f"saved_agents/{config['env']}_20x20/", exist_ok=True)
        ...
        if i % 10 == 9:
            ...
            if score < best_score:
                ...
                torch.save(
                    ...
                    f"saved_agents/{config['env']}_20x20/"
...
def main():
    tuner = Tuner(
        ...
        metric="episode_len",
        mode="min",
        plot_dir="hard_plots_again",
        ckpt_filename="hard_again.ckpt",
    )
