...
fig, axes = plt.subplots(ncols=5, constrained_layout=True)
fig: plt.Figure
...
for i, env, env_title in zip(
    ...
    for jj, method, method_name in zip(
        range(3),
        ("local", "global", "mixed"),
        ("decentralized (MA-GAIL)", "centralized (MA-GAIL)", "mixed (ours)"),
    ):
        science_key = (method,)
...
fig.savefig("discrim_ablation_results.png", bbox_inches="tight")
